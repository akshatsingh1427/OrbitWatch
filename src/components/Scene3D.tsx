...
function SatelliteModel({ satellite, orbitRadius, speed, inclination, phase, selected, onSelect }: SatProps) {
    ...
    <group ref={groupRef}>
      ...
      {selected && (
        <mesh rotation={[Math.PI / 2, 0, 0]}>
          ...
          <meshBasicMaterial color={satellite.color} transparent opacity={0.8} side={THREE.DoubleSide} />
        </mesh>
      )}
    </group>
  );
...
function OrbitRing({ radius, inclination, color }: { radius: number; inclination: number; color: string }) {
  const line = useMemo(() => {
    ...
    const material = new THREE.LineBasicMaterial({ color: new THREE.Color(color), transparent: true, opacity: 0.15 });
    ...
    <>
      ...
      {satConfigs.map(({ satellite, radius, speed, inclination, phase }) => (
        <group key={satellite.id}>
          <OrbitRing radius={radius} inclination={inclination} color={satellite.color} />
          <SatelliteModel
            satellite={satellite}
            orbitRadius={radius}
            speed={speed}
            ...
            onSelect={() =>
              setSelectedSatelliteId(selectedSatelliteId === satellite.id ? null : satellite.id)
            }
          />
        </group>
      ))}
